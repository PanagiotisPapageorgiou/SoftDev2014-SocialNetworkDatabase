...
int appendLists(ListPtr listp1,ListPtr listp2){ //Add all of List 2 behind List 1

	if((listp1 == NULL) || (listp2 == NULL)){
		printf("appendLists: Can't append NULL Lists!\n");
		return 0;
	}

	listp2->Current = listp2->Head;
	while(listp2->Current != NULL){
		addLastList(listp1,listp2->Current->data);
		listp2->Current = listp2->Current->next;
	}
...
int listsAreDuplicated(List listA,List listB,int compareMode){
	...
	if(listA.items <= 0){
		printf("listsAreDuplicated: listA negative or zero items!\n");
		return -1;
	}

	if(listB.items <= 0){
		printf("listsAreDuplicated: listB negative or zero items!\n");
		return -1;
	}

	//printf("listsAreDuplicated: ListA: ");
	//printList(listA);
	//printf("\n");

	//printf("listsAreDuplicated: ListB: ");
	//printList(listB);
	//printf("\n");

	//printf("listsAreDuplicated: Comparing ListA with ListB...\n");

	if(listA.items != listB.items){
		//printf("listsAreDuplicated: Lists are not the same!\n");

		return 0;
	}

	if(strcmp(listA.type_name,listB.type_name)){
		printf("listsAreDuplicated: Can't compare different types of Lists!\n");
		return -1;
	}
	...
	myCurrent = listA.Head;
	while(myCurrent != NULL){
		myCurrent2 = listB.Head;
		while(myCurrent2 != NULL){
			if(compareMode == 1){ //This means simply compare data pointers
				if(myCurrent->data == myCurrent2->data){
					commons++;
				}
			}
			else{
				if((listA.compareNode(myCurrent->data,myCurrent2->data)) == 0){ //Use the appropriate compare function
					commons++;
				}
			}
	...
	if(commons == listA.items){ 
		//printf("listsAreDuplicated: Lists ARE the same!\n");
		return 1;
	}
