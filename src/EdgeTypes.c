...
Edge* setPersonKnowsPersonProperties(int startNodeID, int endNodeID, char* type, int weight, int* errorFlag) { /* Initialise a Person Knows Person Edge */
	...
	//printPersonKnowsPersonProperties(e);
...
Edge* setForumHasMemberProperties(int startNodeID, int endNodeID, char* type, char* joinDate, int* errorFlag) { /* Initialise a Person Knows Person Edge */
    ...
    Edge* e = createEdge(startNodeID, endNodeID, prop, errorFlag);
	 ...
	 //printForumHasMemberProperties(e);
...
Edge* setCliqueKnowsCliqueProperties(int startNodeID, int endNodeID, char* type, int* errorFlag) { /* Initialise a Person Knows Person Edge */

    Properties* prop = createProperties(1,errorFlag); /* Allocated memory for property set */

	if(prop == NULL){
		printf("setCliqueKnowsCliqueProperties: createProperties failed! - Exiting!\n");
		*errorFlag = EXIT_VALUE;
		return ((Edge*) (NULL));
	}

	if(setStringProperty(type, 0, prop) == EXIT_VALUE){ /* Allocated memory for string property */
		printf("setCliqueKnowsCliqueProperties: Error creating Property!\n");
		*errorFlag = EXIT_VALUE;
		destroyProperties(&prop);
		return ((Edge*) (NULL));
	}
	...
	if(*errorFlag == EXIT_VALUE){
		printf("setCliqueKnowsCliqueProperties: createEdge failed! - Exiting!\n");
		if(e != NULL){
			destroyEdge(&e);
		}
		else{
	...
	if(e == NULL){
		printf("setCliqueKnowsCliqueProperties: Error Creating Edge - Exiting!\n");
		*errorFlag = EXIT_VALUE;
		destroyProperties(&prop);
		return ((Edge*) (NULL));
	}
...
int printCliqueKnowsCliqueProperties(Edge* e) {
	...
	if(e != NULL){
		if(e->properties != NULL){
			if(e->properties->arrayP != NULL){
					printf("printCliqueKnowsClique - startID: |%d| - endID: |%d| - Type: |%s|\n", e->startNodeID, e->endNodeID, (char*) e->properties->arrayP[0].attribute);
			}
			else{
				printf("printCliqueKnowsClique: Array of properties is NULL! - Exiting!\n");
				return EXIT_VALUE;
			}			
		}
		else{
			printf("printCliqueKnowsClique: Properties set is NULL! - Exiting!\n");
			return EXIT_VALUE;
		}
	}
	else{
		printf("printCliqueKnowsClique: Edge Pointer is NULL! - Exiting!\n");
		return EXIT_VALUE;
	}
